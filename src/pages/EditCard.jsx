...
import { Plus, X, MapPin, Loader2, Check, CheckCircle2, Info, ShieldCheck } from 'lucide-react';
...
export default function EditCard() {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      // 🌟 UPDATED: Redirects to dashboard after successful update
      navigate('/dashboard'); 
    } catch (err) {
      ...
      <div className="max-w-3xl mx-auto w-full bg-white/80 backdrop-blur-xl rounded-[2.5rem] shadow-[0_8px_40px_rgb(0,0,0,0.08)] p-6 md:p-10 border border-zinc-200/80 relative z-10">
        ...
        <form onSubmit={handleUpdate} className="space-y-8">
            ...
            <div className={`p-6 rounded-2xl border transition-all flex items-center justify-between cursor-pointer shadow-sm hover:shadow-md ${isActive ? 'bg-emerald-50/80 border-emerald-200' : 'bg-red-50/80 border-red-200'}`} onClick={() => setIsActive(!isActive)}>
              <div>
                <h3 className={`font-extrabold text-lg tracking-tight mb-0.5 ${isActive ? 'text-emerald-700' : 'text-red-700'}`}>
