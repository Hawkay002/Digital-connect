...
export default function CreateCard() {
  ...
  const [contacts, setContacts] = useState([{ id: Date.now().toString(), name: '', phone: '', countryCode: '+1', countryIso: 'us', tag: 'Father', customTag: '' }]);
  ...
  useEffect(() => {
    const fetchFamilyId = async () => {
      ...
      if (userDoc.exists()) setFamilyId(userDoc.data().familyId || auth.currentUser.uid);
      else setFamilyId(auth.currentUser.uid);
    };
    fetchFamilyId();
  }, []);
          ...
          <form onSubmit={handleNextStep} className="space-y-6">
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-4 pt-8 mt-8 border-t border-zinc-200/80">
              ...
              <button type="submit" disabled={loading} className={`w-full sm:w-2/3 text-white p-4 rounded-xl font-bold text-lg transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 ${loading ? 'bg-zinc-400 cursor-not-allowed' : 'bg-brandDark hover:bg-brandAccent hover:-translate-y-0.5'}`}>
                ...
                {loading ? 'Securing Identity...' : currentStep === totalSteps - 1 ? 'Save & Generate ID' : <span className="flex items-center gap-2">Continue <ChevronRight size={20} /></span>}
              </button>
            </div>
          </form>
        ) : (
          <div className="text-center space-y-6 py-12 animate-in zoom-in-95 duration-500">
            <div className="w-20 h-20 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center mx-auto mb-6 shadow-sm border border-emerald-100"><Check size={40} strokeWidth={3} /></div>
            ...
            <div className="flex justify-center my-10">
              <div className={`bg-white p-6 rounded-3xl shadow-2xl border-4 ${activeStyle.border} inline-block transform hover:scale-105 transition-transform duration-500`}>
                <QRCodeCanvas id="qr-canvas-create" value={generatedUrl} size={240} level="H" includeMargin={true} fgColor={activeStyle.fg} bgColor={activeStyle.bg} imageSettings={{ src: "/kintag-logo.png", height: 50, width: 50, excavate: true }} />
              </div>
            </div>
            <div className="space-y-4 pt-4 max-w-sm mx-auto">
              <button onClick={downloadQR} className="w-full flex items-center justify-center space-x-2 bg-brandGold text-brandDark p-4 rounded-xl font-bold shadow-[0_4px_20px_rgba(251,191,36,0.3)] hover:shadow-[0_6px_25px_rgba(251,191,36,0.4)] hover:-translate-y-0.5 active:scale-95 transition-all text-lg"><Download size={20} /><span>Download QR Code</span></button>
              <a href={generatedUrl} target="_blank" rel="noreferrer" className="block w-full bg-brandDark text-white p-4 rounded-xl font-bold shadow-md hover:bg-brandAccent hover:-translate-y-0.5 active:scale-95 transition-all text-lg">View Live Card</a>
              <button onClick={() => navigate('/dashboard')} className="block w-full bg-white border border-zinc-200 text-brandDark p-4 rounded-xl font-bold hover:bg-zinc-50 active:scale-95 transition-all shadow-sm">Return to Dashboard</button>
            </div>
          </div>
        )}
      </div>
